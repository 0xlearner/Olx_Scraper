...
#Step 1 complete
provisionPing server-vm 101538 1
...
##### Step 2 complete #####
provisionPing server-vm 101538 2
...
echo ip-172-31-5-67 > /etc/hostname sed -i 's/127.0.0.1.*localhost/127.0.0.1 ip-172-31-5-67.localdomain ip-172-31-5-67 localhost/' /etc/hosts hostname ip-172-31-5-67 
...
# Set Permissions For New User Folder
...
apt install whois
PASSWORD=$(mkpasswd -m sha-512 104b44860d74538f) 
...
#Step 3 complete
provisionPing server-vm 101538 3


# Configure Git Settings
git config --global user.name "olx" && git config --global user.email "zeroxlearner_0x@hotmail.com"
...
# Step 4 complete 
provisionPing server-vm 101538 4
...
provisionPing server-vm 101538 5
